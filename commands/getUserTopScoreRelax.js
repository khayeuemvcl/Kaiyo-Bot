...
module.exports = function (client) {
    client.on("message", async message => {
        if (message.content.startsWith(prefix + "top-r")) {
            var msg = message.content;
            msg = msg.split(prefix + "top-r ");
            msg = msg[1];
            try {
                var user = await query("SELECT * FROM users WHERE username = ?", msg);
                var recentScore = await query("SELECT * FROM scores_relax INNER JOIN beatmaps ON scores_relax.beatmap_md5 = beatmaps.beatmap_md5 WHERE userid = ? AND completed = 3 ORDER BY pp DESC;", user[0].id)
