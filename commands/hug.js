...
const { hugGifs, messages } = require("../constants/const")
    ...
    client.on("message", async message => {
        if (message.content.startsWith(prefix + 'hug')) {
            ...
            if (mention === message.author) return message.channel.send(randomMessage)
                ...
                var gif = hugGifs[Math.round(Math.random() * (hugGifs.length - 1))];
            try {
                const embed = new Discord.RichEmbed()
                .setTitle(`${message.author.username} hugged ${mention.username} awww...`)
                .setImage(gif)
                .setColor('#00ffb3')
